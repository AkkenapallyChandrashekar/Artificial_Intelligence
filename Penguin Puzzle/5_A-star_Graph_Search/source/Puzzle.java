...
public class Puzzle implements Comparable<Puzzle> {
	...
	int cost;//This variable is used to store the cost calculated for the puzzle
	int heuristic;//This variable is used to store the heuristic value of the puzzle
	...
	public Puzzle getClone() {
		...
		tempPuzzle.cost=tempPuzzle.calculateCost();
		tempPuzzle.heuristic=0;
		return tempPuzzle;
	}
//This method helps us to read each value of character array and store it in a String text
	...
	//The calculateHeuristic Function helps to find the heuristic value of the puzzle object. 
	public int calculateHeuristic() {
		int heuristic = 0;
			if(this.isPenguEatenByBearOrShark()) { //Here if the state of the penguin is dead then I am trying to assign the least possible number as heuristic value of puzzle
				heuristic=Integer.MIN_VALUE;    
			}
			heuristic+=this.stepsTaken.length()*-5; //Here the heuristic value is proportionately decreased based on path length. The more the path length the less the heuristic
			heuristic+=this.findFishCount()*-10;// Here the heuristic value is modified based on the fishes count. The more the fishes count the less the heuristic value
		return heuristic;//The heuristic value is returned.
	}
    //The calculateCost function helps to find the cost value of the puzzle object
	public int calculateCost() {
		int cost = 0;
		if(this.isPenguEatenByBearOrShark()==false) { //Here the cost would be calculated based on the whether the penguin is dead or alive, if alive then more cost if not 
			heuristic+=this.cost*10;    // then less.
		}
		cost += this.fishesEatenCount*20;//Here the cost value is modified proportionately with respect to the number of fishes consumed by penguin
		return cost;
	}
	public int compareTo(Puzzle state2) {//The compareTo function is internally called which helps to internally sort the Priority Queue 
		int state1val=this.heuristic+this.cost;
		int state2val=state2.heuristic+state2.cost;
		int val=Integer.compare(state1val, state2val);
		return val;
